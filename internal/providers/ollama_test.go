...
func TestGetGGUFQuantizationsIntegration(t *testing.T) {
	if testing.Short() {
		t.Skip("Skipping integration test in short mode")
	}
	...
	tests := []struct {
		...
		{"Qwen/Qwen2.5-3B-Instruct-GGUF", []string{"Q4_K_M", "Q4_K_S", "Q5_K_M"}},
	}

	for _, tt := range tests {
		t.Run(tt.modelID, func(t *testing.T) {
			quants, err := GetGGUFQuantizations(ctx, tt.modelID)
			if err != nil {
				t.Skipf("failed to fetch quantizations for %s: %v", tt.modelID, err)
			}

			if len(quants) == 0 {
				t.Error("expected at least one quantization")
			}

			t.Logf("Model %s: found %d quantizations", tt.modelID, len(quants))
			for _, q := range quants {
				t.Logf("  - %s", q)
			}

			for _, want := range tt.wantQuants {
				found := false
				for _, q := range quants {
					if q == want {
						found = true
				...
				if !found {
					t.Logf("expected quantization %s not found (may be acceptable)", want)
				}
			}
		})
	}
