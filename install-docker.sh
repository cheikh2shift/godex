...
echo ""
echo "  UID=\\${UID:-\\$(id -u)} GID=\\${GID:-\\$(id -g)} WORKSPACE_DIR=\"\\${PWD}\" docker compose -f ${INSTALL_DIR}/docker-compose.yml up -d && docker attach godex && docker compose -f ${INSTALL_DIR}/docker-compose.yml down"
echo ""
echo "  # Add this alias to ~/.bashrc or ~/.zshrc:"
echo "  alias godex='UID=\\${UID:-\\$(id -u)} GID=\\${GID:-\\$(id -g)} WORKSPACE_DIR=\"\\${PWD}\" docker compose -f ${INSTALL_DIR}/docker-compose.yml up -d && docker attach godex && docker compose -f ${INSTALL_DIR}/docker-compose.yml down'"
echo ""
echo "  # Or add this function for clearer behavior:"
echo "  godex() {"
echo "    UID=\\${UID:-\\$(id -u)} GID=\\${GID:-\\$(id -g)} WORKSPACE_DIR=\"\\${PWD}\" docker compose -f ${INSTALL_DIR}/docker-compose.yml up -d \\"
